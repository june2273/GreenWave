...
`3x2-brt --traffic high` 는 6 교차로 (성금/한누리-도움4/어진/청사/갈매-도움4/갈매로-가름로) 매핑 기반의 학술용 최종 시나리오. `evaluate_mappo.py --baseline sejong` 와 함께 사용 시 세종 실측 신호 타이밍 (per-TLS 비대칭) 대비 평가 가능.
...
```bash
# 기본 (균등 사이클 Fixed-Time 베이스라인)
python evaluate_mappo.py --model models/MAPPO_sumo_N --map 2x2 --traffic high --episodes 5

# 세종시 실측 신호 베이스라인 (3x2-brt 최종 평가용)
python evaluate_mappo.py \
  --model      models/MAPPO_sumo_N \
...
`--baseline sejong` : `evaluate_mappo.py` 내 `SEJONG_PER_TLS_PHASE_SECONDS` dict 의 per-TLS 실측 신호 적용 (성금/청사/도움4/어진/갈매로-가름 등 4-phase 비대칭 타이밍, 1순위 세담터 2026.04 + 2순위 공공데이터포털 2023). 추가로 `--sejong-phase-seconds 33 20 33 20` 으로 4 phase 단일 set 강제도 가능.

결과는 `results/eval_metrics_mappo_N.csv` 에 저장 (에피소드별 raw + 요약 통계 + 모델 메타데이터 prefix).
