...
**`map_presets.py`** — `--map` 옵션을 sumo_cfg 경로 + default tls_ids로 해상. 사용자 명시 `--sumo-cfg` 가 preset 보다 우선. `2x2 + --traffic high` 조합은 dense routes 사용 (legacy 호환). `--tls-ids` CLI 인자는 cleanup 으로 제거됨 (모든 시나리오 preset 사용).
