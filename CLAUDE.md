...
python evaluate_mappo.py --model models/MAPPO_sumo_1 --map 2x2-brt --sample  # Categorical sampling instead of argmax
...
python evaluate_mappo.py \
  ...
  --baseline sejong
# → per-TLS 실측 신호 자동 적용 (SEJONG_PER_TLS_PHASE_SECONDS dict, 6 TLS 각각 다른 cycle):
...
# 출처: 세담터(2026.04) 가름로, 공공데이터포털(2023) 성금/청사/어진, 도움4로는 corridor 추정.
# 모든 agent에 동일 신호를 적용하려면:
python evaluate_mappo.py ... --baseline sejong --sejong-phase-seconds 47 20 47 28
...
python record_video_mappo.py --model models/MAPPO_sumo_1 --map 2x2-brt --mode continuous  # default, ~3600 frames
python record_video_mappo.py --model models/MAPPO_sumo_1 --map 2x2-brt --mode short       # env.step() 당 1 frame (~720 frames)
python record_video_mappo.py --model models/MAPPO_sumo_1 --map 2x2-brt --sample          # Categorical sampling instead of argmax
...
- `--brt-weight w` (default 1.0): vClass=bus 차량의 waiting time 만 w 배로 가중 (`current_wait = Σ_veh w_veh × accum_wait / 10`). 1.0 = baseline (lane.getWaitingTime 합과 수학적으로 동일, fast-path 로 vehicle 단위 traci 호출 회피). BRT 시나리오 (2x2-brt, 3x2-brt) 에서만 의미. info dict 의 `avg_wait_brt/car`, `avg_speed_brt/car`, `brt_seen`, `car_seen` 는 w 와 무관하게 항상 기록 (baseline vs weighted 직접 비교 용이).
...
**`evaluate_mappo.py`** — 3-way comparison: MAPPO vs CTDE-MAPPO (optional) vs Fixed-time. Results saved to `results/eval_metrics_mappo_N.csv` with metadata prefix columns for model traceability. `--sample` flag uses Categorical sampling instead of argmax (reveals true action distribution; useful for detecting mode collapse). `--baseline` 선택: `symmetric` (균등 N-step 사이클, 기본) / `sejong` (세종 실측 per-TLS 비대칭 신호 — `SEJONG_PER_TLS_PHASE_SECONDS` dict 의 6 TLS 매핑 사용, 1순위 세담터 2026 + 2순위 공공데이터포털 2023). `--sejong-phase-seconds` 4 값 명시 시 모든 agent 에 동일 set 강제.
...
- `3x2_brt/` — 6 TLS, 좌측 col vertical 3-lane BRT corridor. 좌측 TLS (1,3,5) 14-link, 우측 TLS (2,4,6) 12-link. **세종시 현실 시뮬레이션 최종 시나리오.**
  - `routes_3x2_brt.rou.xml` (default, ~6,120 veh/h, probability 기반): BRT 6분 배차 (10대/h/방향).
  - `routes_3x2_brt_dense.rou.xml` (`--traffic high`, ~8,530 veh/h): **행복청 제23차 행복도시 교통량 조사(2025.04) 1차 + 공공데이터포털 2023 2차** 기반. TLS 매핑 — 1=성금, 2=청사, 3=한누리-도움4(추정), 4=갈매로-도움4(추정), 5=어진, 6=갈매로-가름로. v1 BRT corridor=980 / v2=470 / h1=h2=h3=750 veh/h/방향 (각 ±10% 추정 보정). cross 도시 좌회전 15% 비율 (375/240/480/240). BRT 6분 배차 (period=360).
...
|-----------|-------|------|
| `lr` | `1e-4` | policy 진동 억제 |
| `train_batch_size` | `8000` | gradient noise 감소 |
| `num_epochs` | `6` | — |
| `minibatch_size` | `128` | — |
| `gamma` | `0.99` | — |
| `lambda_` | `0.95` | GAE λ |
