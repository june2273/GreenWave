...
- Reward: `--reward-mode` 로 2종 선택. **`diff-waiting-time`(기본)** — `(prev − current) / 10`, 차분(Δ) 기반, 자기 진입로 누적 대기시간만 봄. **`pressure`** — `Σ#(out_lanes 차량) − Σ#(in_lanes 차량)`, max-pressure/backpressure 보상, 상태기반(차분 아님), **하류 포화를 직접 반영**해 과포화(LOS D+) 스필백·grid lock·teleport 억제 (Varaiya 2013, PressLight 2019). `switch_penalty=0.45` (phase switching 시 reward -= 0.45, oscillation 억제, yellow_time=3s 기준, 두 모드 공통). **주의**: pressure 는 스케일이 ±수십 (diff-waiting 의 ~수천 대비 작음)이라 switch_penalty 0.45 가 상대적으로 과해짐 → pressure 학습 시 `--switch-penalty 0.1` 전후 권장. pressure 는 차량 *수* 기반이라 `brt_weight` 미적용 (diff-waiting 전용). out_lanes 는 probe 단계(`per_agent_out_lanes`)에서 이미 계산됨. diff-waiting vs pressure 동일 하이퍼파라미터 ablation 가능.
