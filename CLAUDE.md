...
python train_mappo.py --map 2x2-brt --ctde --ctde-reward local  # CTDE with per-agent reward
...
- Reward: `diff-waiting-time` 단일 모드 — `(prev − current) / 10`. `switch_penalty=0.3` (phase switching 시 reward -= 0.3, oscillation 억제). queue/pressure 모드는 cleanup 으로 제거됨.
...
**`map_presets.py`** — `--map` 옵션을 sumo_cfg 경로 + default tls_ids로 해상. 사용자 명시 `--sumo-cfg` 가 preset 보다 우선. `--traffic high` 분기: `2x2` → `2x2grid_dense.sumocfg` (legacy), `2x2-brt` → `2x2_brt_dense.sumocfg` (세종시 실측 ~6,130 veh/h). `--tls-ids` CLI 인자는 cleanup 으로 제거됨 (모든 시나리오 preset 사용).
...
- `2x2_brt/` — 4 TLS, 좌측 col vertical edges 3-lane (lane 2 = `allow="bus"`). BRT corridor TLS는 14-link state, 표준 TLS는 12-link.
  - `routes_2x2_brt.rou.xml` (default, ~3,650 veh/h): 세종시 실측 분포의 0.59x 스케일. 빠른 학습 iteration & 좌회전 phase 학습용.
  - `routes_2x2_brt_dense.rou.xml` (`--traffic high`, ~6,130 veh/h): **행복청 제23차 행복도시 교통량 조사(2025.04) AM peak 실측치 직접 적용** (성금/어진/청사/갈매로-가름로 4교차로). v1(BRT corridor)=980, v2(우측열)=470 veh/h/방향. BRT 6분 배차. LOS D 조건.
